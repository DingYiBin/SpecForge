...
def build_target_model(
    ...
    raw_target_config = None
    if model_type is None:
        raw_target_config = _raw_target_config(args)
        model_type = raw_target_config.get("model_type")
    if model_type == "deepseek_v4":
        if expert_dtype is None:
            raw_target_config = raw_target_config or _raw_target_config(args)
            expert_dtype = raw_target_config.get("expert_dtype")
        # SGLang 0.5.14's DeepSeek-V4 pool uses FP8 NoPE KV and keeps only 10%
        # of max_total_tokens for SWA by default, which is too small for this
        # full-sequence offline prefill.
        sglang_kwargs.update(
            kv_cache_dtype="fp8_e4m3",
            swa_full_tokens_ratio=1.0,
            _force_cuda_norm=True,
        )
    if expert_dtype == "fp4":
    ...
    print_with_rank(
        ...
        f"kv_cache_dtype={sglang_kwargs.get('kv_cache_dtype', 'auto')!r}"
    )
